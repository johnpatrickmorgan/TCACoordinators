...
struct FormAppCoordinatorView: View {
  ...
  var body: some View {
    TCARouter(store) { screen in
      SwitchStore(screen) {
        CaseLet(state: /FormScreen.State.step1, action: FormScreen.Action.step1, then: Step1View.init(store:))

        CaseLet(state: /FormScreen.State.step2, action: FormScreen.Action.step2, then: Step2View.init(store:))

        CaseLet(state: /FormScreen.State.step3, action: FormScreen.Action.step3, then: Step3View.init(store:))

        CaseLet(state: /FormScreen.State.finalScreen, action: FormScreen.Action.finalScreen, then: FinalScreenView.init(store:))
      }
    }
  }
}
